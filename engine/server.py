...
Run locally with: python engine/server.py
Then open the GitHub Pages UI. The UI talks only to localhost.
No PII is sent to DeleteMe servers.
"""
...
def load_brokers() -> list[dict[str, Any]]:
    if not BROKERS.exists():
        raise HTTPException(503, "Broker registry missing. Run: python engine/sync_brokers.py")
...
def text_tokens(profile: Profile) -> dict[str, str]:
    return {
        ...
        "full": f"{profile.first_name} {profile.last_name}",
        "email": profile.email, "phone": profile.phone,
        "address": profile.address, "city": profile.city,
        "state": profile.state, "postal": profile.postal_code,
        "zip": profile.postal_code, "country": profile.country,
    }
...
async def automate_page(page, broker: dict, profile: Profile) -> tuple[str, str]:
    ...
    if not url:
        return "manual", "No web opt-out URL; use the broker email route."
    try:
        await page.goto(url, wait_until="domcontentloaded", timeout=30000)
        # Fill only obvious identity fields. Never guess sensitive verification fields.
        ...
        for i in range(min(await buttons.count(), 30)):
            b = buttons.nth(i)
            txt = ((await b.inner_text()) + " " + (await b.get_attribute("value") or "")).lower()
...
def send_email(broker: dict, profile: Profile, smtp: dict) -> tuple[str, str]:
    ...
    msg["Subject"] = "Personal Data Deletion / Opt-Out Request"
...
async def run_all(profile: Profile):
    ...
    with LOCK:
        RUN.update(running=True, total=len(brokers), completed=0, submitted=0, manual=0, failed=0, items=[])
    smtp = None
    ...
    async with async_playwright() as pw:
        ...
        for broker in brokers:
            ...
            try:
                if smtp and broker.get("optOutEmail"):
                    status, detail = await asyncio.to_thread(send_email, broker, profile, smtp)
                else:
                    page = await context.new_page()
                    status, detail = await automate_page(page, broker, profile)
                    if status != "submitted":
                        # Keep manual/CAPTCHA page open for the user.
                        pass
                    else:
                        await page.close()
            ...
            with LOCK:
                ...
                RUN["items"].append({"company": company, "domain": domain, "status": status, "detail": detail})
        await browser.close()
...
def remove_all(req: StartRequest):
    ...
    init_db()
    threading.Thread(target=lambda: asyncio.run(run_all(req.profile)), daemon=True).start()
    return {"started": True}
